<template>
  <section class="graph-page">
    ...
    <div class="graph-body" v-else>
      ...
      <!-- Fixed-height info dock at the bottom. -->
      <div class="graph-dock">
        <template v-if="selectedNode">
          <div class="dock-main">
            <div class="dock-head">
              <span class="dock-type">{{ selectedNode.type || "file" }}</span>
              <span class="dock-title">{{ selectedNode.title }}</span>
              <span class="dock-counts">{{ selectedNode.outbound }} out · {{ selectedNode.inbound }} in</span>
              <a class="btn ghost btn-sm" :href="rawUrl(selectedNode.path)" target="_blank" rel="noopener">
...
// library) rather than a hand-rolled simulation: it gives correct tap-vs-drag
// handling, pan/zoom, and a force layout for free. The graph fills the
// available width/height; a fixed-height dock at the bottom shows the selected
...
function buildGraph(): void {
  ...
  const elements: cytoscape.ElementDefinition[] = [
    ...
    // missing) are surfaced in the dock instead.
    ...edges.value
    ...
    style: [
      {
        ...
        style: {
          ...
          "text-valign": "bottom",
          "text-margin-y": 3,
          width: "mapData(deg, 0, 12, 14, 46)",
          height: "mapData(deg, 0, 12, 14, 46)",
...
/* The graph takes all remaining space horizontally and vertically. */
.graph-canvas {
  flex: 1 1 auto;
  min-height: 260px;
  width: 100%;
...
/* Fixed-height dock pinned below the graph. */
.graph-dock {
  flex: 0 0 auto;
  height: 150px;
  margin-top: 10px;
